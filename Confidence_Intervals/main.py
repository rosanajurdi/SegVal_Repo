...
from Confidence_Intervals.Statistical_functions import (
    statistical_analysis,
    create_subsampling_data,
    Bootstrap_Analysis,
)
...
def plot_dist(data, metric_name, name, path):
    ...
    plt.savefig(
        os.path.join(
            path[0], "Distribution-{}-{}-{}.svg".format(name, framework, metric_name)
        )
    )


def write_to_overleaf(df_sample, txt_file_Boot, k):
    mean_variation = "{} $\pm$ {} ".format(
        df_sample["u-kj"].mean().round(3), df_sample["u-kj"].std().round(3)
    )
    std_variation = "{} $\pm$ {}".format(
        df_sample["sigma-kj"].mean().round(3), df_sample["sigma-kj"].std().round(3)
    )
    SEM_Variation = "{} ".format(df_sample["SEM-kj"].mean().round(3))
    Width_Variation = "{} $\pm$ {}".format(
        df_sample["w-kj"].mean().round(3), df_sample["w-kj"].std().round(3)
    )
    width_k = df_sample["w-kj"].mean().round(2)
    CI = [-width_k / 2, width_k / 2]

    mean_variation_star = "{} $\pm$ {} ".format(
        df_sample["u-kj-star"].mean().round(3), df_sample["u-kj-star"].std().round(3)
    )
    SEM_Variation_star = "{}".format(df_sample["SEM-kj-star"].mean().round(3))
    Width_Variation_star = "{} \pm {}".format(
        df_sample["w-kj-star"].mean().round(3), df_sample["w-kj-star"].std().round(3)
    )
    CI_star = "[{}, {}]".format(
        df_sample["A-kj-star"].mean(), df_sample["B-kj-star"].mean()
    )
...
def run(args: argparse.Namespace):
    ...
    mu_star, sem_star, w_star, conf_interval, conf_interval_norm_star = (
        Bootstrap_Analysis(data, skewness=True)
    )
    ...
    full_result.write(
        str(
            "stats and bootstrapp: {}  &  {}  &  {}  &  [-{}, {}]  & {} &  {} &  [-{}, {}]".format(
                ...
                mu_star,
                sem_star,
                conf_interval_norm_star[0],
                conf_interval_norm_star[1],
            )
        )
    )
    ...
    if subsampling == True:
        ...
        df = pd.DataFrame(
            data=listt, columns=["k", "Mean", "STD", "SEM", "Width", "CI"]
        )
        writer = pd.ExcelWriter(
            os.path.join(save_dir, "results.xlsx"), engine="xlsxwriter"
        )
...
def get_args(root, dataset) -> argparse.Namespace:
    ...
    parser.add_argument("--K_samples", type=list, default=k_dict[dataset])
    ...
    for framework in frameworks:
        for root, exp, csv_files in os.walk(os.path.join(stats_path, framework)):
            for exp in csv_files:
                ...
                if (
                    i > 0
                    and os.path.splitext(exp)[1] == ".csv"
                    and "subsampled-stats" not in exp
                ):
