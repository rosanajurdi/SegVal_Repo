...
def create_subsampling_data(K_samples, data, save_dir, name, metric_name):
    ...
    for k in K_samples:
        ...
        for j in tqdm(range(0,100)): # the experiment needs to be repeated a 100 times since in small samples the values will highly depend on sampled data.
            ...
            GCI_norm = [-np.round( 1.96 * std_kj / np.sqrt(k), 4), np.round( + 1.96 * std_kj / np.sqrt(k), 4)]
            ...
            mean_kj_star, std_kj_star, w_kj_star, BCI, BCI_norm = Bootstrap_Analysis(dice_acccuracies_Skj)
...
def Bootstrap_Analysis(data, k= None, skewness = True):
    ...
    if skewness:
        ...
        conf_interval_norm = np.array(np.percentile(Norm_bs_replicates, [2.5, 97.5]).round(2))
    ...
    return np.round(mu_star, 3), np.round(sem_star,3),  np.round(w_star, 3), np.round(conf_interval,3), np.round(conf_interval_norm, 3)
