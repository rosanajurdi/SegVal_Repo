...
def create_subsampling_data(K_samples, data, save_dir, name, metric_name):
    ...
    for k in K_samples:
        ...
        for j in tqdm(
            range(0, 100)
        ):  # the experiment needs to be repeated a 100 times since in small samples the values will highly depend on sampled data.
            dice_acccuracies_Skj = data["metric"].sample(
                k
            )  # N samples were selected without replacement
            ...
            w_kj = 2 * 1.96 * sem_kj
            GCI = [
                np.round(mean_kj - 1.96 * std_kj / np.sqrt(k), 4),
                np.round(mean_kj + 1.96 * std_kj / np.sqrt(k), 4),
            ]
            GCI_norm = [
                -np.round(1.96 * std_kj / np.sqrt(k), 4),
                np.round(+1.96 * std_kj / np.sqrt(k), 4),
            ]
            ...
            Set_GCI_0.append(GCI_norm[0])
            Set_GCI_1.append(GCI_norm[1])

            # Bootstrapping Analysis of the sub-samples
            mean_kj_star, std_kj_star, w_kj_star, BCI, BCI_norm = Bootstrap_Analysis(
                dice_acccuracies_Skj
            )

            SET_u_star_k_j.append(mean_kj_star)
            SET_SEM_star_k_j.append(std_kj_star)
            SET_W_kj_star.append(w_kj_star)
            SET_BCI_0.append(BCI_norm[0])  # the values are independent of the mean
            SET_BCI_1.append(BCI_norm[1])

    sample_data = {
        "sample-set": SET_k,
        "u-kj": SET_u_k_j,
        "sigma-kj": SET_sigma_k_j,
        "SEM-kj": SET_SEM_k_j,
        "w-kj": SET_W_kj,
        "A-kj": Set_GCI_0,
        "B-kj": Set_GCI_1,
        "u-kj-star": SET_u_star_k_j,
        "SEM-kj-star": SET_SEM_star_k_j,
        "w-kj-star": SET_W_kj_star,
        "A-kj-star": SET_BCI_0,
        "B-kj-star": SET_BCI_1,
    }
...
def draw_bs_replicates(data, func, size):
    ...
    for i in range(size):
        ...
        bs_sample = np.random.choice(
            data, size=len(data)
        )  # drawing a k sampled size with replacement
...
def Bootstrap_Analysis(data, k=None, skewness=True):
    ...
    dice_acccuracies = (
        data["metric"].sample(k).reset_index(drop=True)
    )  # k samples were selected without replacement when its k =110 samples for hippo or k = 334 for brain, the sampls set remains unchanged from the test set
    if k is None:
    ...
    bs_replicates = draw_bs_replicates(
        dice_acccuracies, np.mean, 15000
    )  # sample 15000 times from the subset of size n with replacement.
    ...
    if skewness:
        # did visualization to verify that it is good
        Norm_bs_replicates = bs_replicates - mu_star
        conf_interval_norm = np.array(
            np.percentile(Norm_bs_replicates, [2.5, 97.5]).round(2)
        )
