...
def Generate_gaussian_dist_excel(save_dir):
        ...
        for s in sigma:
            ...
            listt.append(
                [
                    k,
                    s,
                    np.round(s / np.sqrt(k), 2),
                    np.round(2 * 1.96 * s / np.sqrt(k), 2),
                    ci,
                ]
            )
    ...
    writer = pd.ExcelWriter(
        os.path.join(save_dir, "gaussian_excel.xlsx"), engine="xlsxwriter"
    )
...
if __name__ == "__main__":
    Generate_gaussian_dist_table(
        "/Users/rosana.eljurdi/PycharmProjects/SegVal_Project/Confidence_Intervals"
    )
