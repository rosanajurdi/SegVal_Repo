    ...
    for k in K_samples:
        ...
        if k < K_samples[-1]:
            ...
            std_variation = "${} \pm {}$".format(
                df_sample["sigma-kj"].mean().round(3),
        ...
        CI = [
            -width_k / 2,
            width_k / 2,
        ]  # this needs to be symmetric since it follows the gaussian distribution

        mean_variation_star = "${} \pm {}$ ".format(
            df_sample["u-kj-star"].mean().round(3),
        ...
        Width_star_norm = df_sample["w-kj-star"].mean().round(3) / df_sample[
            "u-kj-star"
        ].mean().round(3)
        CI_star = "$[{}, {}]$".format(
            df_sample["A-kj-star"].mean().round(3),
...
def get_caption(path):
    ...
    caption_1 = "{}, {}, {}. Results on subsamples of size $k \leq {}$. Results are shown as mean".format(
        dataset, framework, metric, k
    )
    caption_2 = "$\pm$ std where mean and std are the mean and standard-deviation of the {}".format(
        metric
    )
    caption_3 = "over all the subsamples Sk,j of a given size k (k is fixed and $j \in [1, . . . , n]$) with n = {}".format(
        k
    )
    label_t = "table:{}-{}-{}".format(
        label[p.parts[7]], p.parts[8].split("-")[2], framework.split("-")[0]
    )
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        choices=["hippo", "brain"],
        help="Specify the key (hippo or brain)",
    )
    args = parser.parse_args()
    print(args)
    return args
